...
class HandEvaluator {
     ...
     * @returns The score of the hand.
     */
    static compareCardsOfSameTieBreaker(cardsOne, cardsTwo, comparison) {
        switch (comparison) {
            case "Royal Flush":
                return 0;
            case "Straight Flush":
            ...
            case "Two Pair":
                ...
                if (higherPairCompare !== 0)
                    return higherPairCompare;
                let lowerPairCompare = cardsTwo[2].compare(cardsOne[2]);
                if (lowerPairCompare !== 0)
                    return lowerPairCompare;
                return cardsTwo[4].compare(cardsOne[4]);
            case "One Pair":
    ...
    static calculateScore(hand, communityCards) {
        const allCards = [...hand, ...communityCards];
        const royalFlush = this.isRoyalFlush(allCards);
        if (royalFlush && royalFlush.length)
            return { score: 10, tiebreaker: royalFlush, comparison: "Royal Flush" };
        const straightFlush = this.isStraightFlush(allCards);
        if (straightFlush && straightFlush.length)
            return { score: 9, tiebreaker: straightFlush, comparison: "Straight Flush" };
        const fourOfAKind = this.isFourOfAKind(allCards);
        if (fourOfAKind && fourOfAKind.length)
            return { score: 8, tiebreaker: fourOfAKind, comparison: "Four of a Kind" };
        const fullHouse = this.isFullHouse(allCards);
        if (fullHouse && fullHouse.length)
            return { score: 7, tiebreaker: fullHouse, comparison: "Full House" };
        const flush = this.isFlush(allCards);
        if (flush && flush.length)
            return { score: 6, tiebreaker: flush, comparison: "Flush" };
        const straight = this.isStraight(allCards);
        if (straight && straight.length)
            return { score: 5, tiebreaker: straight, comparison: "Straight" };
        const threeOfAKind = this.isThreeOfAKind(allCards);
        if (threeOfAKind && threeOfAKind.length)
            return { score: 4, tiebreaker: threeOfAKind, comparison: "Three of a Kind" };
        const twoPair = this.isTwoPair(allCards);
        if (twoPair && twoPair.length)
            return { score: 3, tiebreaker: twoPair, comparison: "Two Pair" };
        const pair = this.isPair(allCards);
        if (pair && pair.length)
            return { score: 2, tiebreaker: pair, comparison: "One Pair" };
        return { score: 1, tiebreaker: this.getHighCards(allCards), comparison: "High Card" };
    }
    static compareSequentially(cardsOne, cardsTwo) {
